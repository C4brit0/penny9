/**
 * Custom Script for penny9squad cytu.be channel
 * Original functionality preserved - Organized and commented
 */
...
// =============================================
// SISTEMA DE PM PERSONALIZADO
// =============================================

function customizePMs() {
    // Aplica classes personalizadas a todas as PMs existentes
    document.querySelectorAll('.pm-wrap').forEach(pm => {
        pm.classList.add('custom-pm');
    });
    
    document.querySelectorAll('.pm-header').forEach(header => {
        header.classList.add('custom-pm-header');
    });
    
    document.querySelectorAll('.pm-history-content').forEach(content => {
        content.classList.add('custom-pm-messages');
    });
    
    document.querySelectorAll('.pm-input').forEach(input => {
        input.classList.add('custom-pm-input');
    });
    
    // Observa novas PMs que forem abertas
    const observer = new MutationObserver(function(mutations) {
        mutations.forEach(function(mutation) {
            mutation.addedNodes.forEach(function(node) {
                if (node.nodeType === 1) {
                    // Verifica se é uma PM nova
                    const pmWrap = node.classList?.contains('pm-wrap') ? node : node.querySelector?.('.pm-wrap');
                    const pmHeader = node.classList?.contains('pm-header') ? node : node.querySelector?.('.pm-header');
                    const pmContent = node.querySelector?.('.pm-history-content');
                    const pmInput = node.querySelector?.('.pm-input');
                    
                    if (pmWrap) pmWrap.classList.add('custom-pm');
                    if (pmHeader) pmHeader.classList.add('custom-pm-header');
                    if (pmContent) pmContent.classList.add('custom-pm-messages');
                    if (pmInput) pmInput.classList.add('custom-pm-input');
                }
            });
        });
    });
...
$(document).ready(function() {
    setTimeout(customizePMs, 1000);
});

// Detecta novas mensagens PM para aplicar highlight
socket.on('pm', function(data) {
    setTimeout(function() {
        document.querySelectorAll('.pm-wrap').forEach(pm => {
            pm.classList.add('highlight');
        });
        document.querySelectorAll('.pm-header').forEach(header => {
...
// Remove highlight quando o usuário clicar na PM
document.addEventListener('click', function(e) {
    if (e.target.closest('.pm-wrap')) {
        e.target.closest('.pm-wrap').classList.remove('highlight');
        const header = e.target.closest('.pm-wrap').querySelector('.pm-header');
        if (header) header.classList.remove('highlight');
    }
});
